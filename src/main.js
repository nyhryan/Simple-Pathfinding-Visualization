...
const colsInput = document.querySelector("input#columns");
let ROW = Number(rowsInput.value);
let COL = Number(colsInput.value);
const rowsOutput = document.querySelector("#rowValueLabel");
const colsOutput = document.querySelector("#colValueLabel");
rowsOutput.innerHTML = String(ROW);
colsOutput.innerHTML = String(COL);
const gridRowColInput = document.querySelector("#row-column-input");
...
const fillGrid = () => {
    for (let r = 0; r < ROW; r++) {
        for (let c = 0; c < COL; c++) {
            ...
            ctx.fillRect(c * cellSize + cellGap, r * cellSize + cellGap, cellSize - cellGap * 2, cellSize - cellGap * 2);
        }
    }
};
let gridX = 0, gridY = 0;
const convertToGridPos = (e) => {
...
const drawModeButtons = document.querySelector("#draw-mode");
drawModeButtons.addEventListener("click", () => {
    let drawModeLabel = document.querySelector("#draw-mode-label");
    switch (drawMode) {
        case 0:
...
const validateGrid = () => {
    ...
    if (startNodeCount === 1 && goalNodeCount === 1)
        return true;
    return false;
};
class GridNode {
...
const traceRouteBFS = async (start, goal, output) => {
    ...
    while (!isSameNode(currentNode, start)) {
        ...
        if (!isSameNode(currentNode, start))
...
const traceRoute = async (start, goal, output) => {
    let currentNode = goal;
    let stack = [];
    let iter = 1;
    while (!isSameNode(currentNode, start)) {
...
const BFS = async (start, goal, map) => {
    let queue = [];
    queue.push(start);
    const BFSLoop = async () => {
...
const GBFS = async (start, goal, map) => {
    let queue = new Map();
...
const ASTAR = async (start, goal, map) => {
    let queue = new Map();
